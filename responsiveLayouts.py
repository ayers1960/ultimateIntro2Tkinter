...
class App(tk.Tk):
    ...
    def create_medium_layout(self):
        print ("create_medium_layout")
        self.frame.pack_forget()
        self.frame = ttk.Frame(self)
        self.frame.columnconfigure((0,1), weight = 1, uniform = 'a')
        self.frame.rowconfigure((0,1), weight=1, uniform = 'a')
        self.frame.pack(expand = True, fill ='both')
        ttk.Label(self.frame, text = 'Label-1', background = 'red').grid(column=0, row=0,    sticky='news', padx=5, pady=5)
        ttk.Label(self.frame, text = 'Label-2', background = 'green').grid(column=1, row=0,  sticky='news', padx=5, pady=5)
        ttk.Label(self.frame, text = 'Label-3', background = 'blue').grid(column=0, row=1,   sticky='news', padx=5, pady=5)
        ttk.Label(self.frame, text = 'Label-4', background = 'yellow').grid(column=1, row=1, sticky='news', padx=5, pady=5)
        print("medium")
    ...
    def create_large_layout(self):
        ...
        self.frame.pack_forget()
        self.frame = ttk.Frame(self)
        ttk.Label(self.frame, text = 'Label-1', background = 'red').pack(  side='left', expand=True, fill='both', padx= 10, pady=5)
        ttk.Label(self.frame, text = 'Label-2', background = 'green').pack( side='left', expand=True, fill='both', padx= 10, pady=5)
        ttk.Label(self.frame, text = 'Label-3', background = 'blue').pack( side='left', expand=True, fill='both', padx= 10, pady=5)
        ttk.Label(self.frame, text = 'Label-4', background = 'yellow').pack( side='left', expand=True, fill='both', padx= 10, pady=5)
        self.frame.pack(expand=True, fill='both')        
